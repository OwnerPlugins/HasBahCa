...
echo "Starting HasBahCa installation..."
...
cleanup() {
    ...
    [ -d "/tmp/HasBahCa-main" ] && rm -rf "/tmp/HasBahCa-main"
}
...
echo "Downloading HasBahCa..."
wget --no-check-certificate 'https://github.com/Belfagor2005/HasBahCa/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "Failed to download HasBahCa package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "Failed to extract HasBahCa package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/HasBahCa-main/usr/lib/enigma2/python/Plugins/Extensions/HasBahCa" ]; then
    cp -r "$TMPPATH/HasBahCa-main/usr/lib/enigma2/python/Plugins/Extensions/HasBahCa"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from standard plugin directory"
elif [ -d "$TMPPATH/HasBahCa-main/usr/lib64/enigma2/python/Plugins/Extensions/HasBahCa" ]; then
    cp -r "$TMPPATH/HasBahCa-main/usr/lib64/enigma2/python/Plugins/Extensions/HasBahCa"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/HasBahCa-main/usr" ]; then
    cp -r "$TMPPATH/HasBahCa-main/usr"/* /usr/ 2>/dev/null
...
bbox_type=$(sed -n '1p' /etc/hostname 2>/dev/null || echo "Unknown")
...
#########################################################
#          HasBahCa $version INSTALLED SUCCESSFULLY      #
